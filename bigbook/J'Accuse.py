...
ITEMS = ['FLASHLIGHT', 'CANDLESTICK', 'RAINBOW FLAG', 'HAMSTER WHEEL', 'ANIME VHS TAPE',
         'JAR OF PICKLES', 'ONE COWBOY BOOT', 'CLEAN UNDERPANTS', '5$ GIFT CARD']
...
while True:  # Main game loop.
    if time.time() > endTime or accusationsLeft == 0:
        ...
        print(f'It was {culprit} at the {PLACES[culpritIndex]} with the '
              f'{ITEMS[culpritIndex]} who catnapped her!')
    ...
    if response == 'J':  # Player accuses this suspect.
        ...
        if thingBeingAskedAbout in (thePersonHere, theItemHere):
            ...
            print('  They gie you this clue:"{}"'
                  .format(clues[thePersonHere][thingBeingAskedAbout]))
            # Add non-place clues to the list of known things:
            if (clues[thePersonHere][thingBeingAskedAbout] not in
                knownSuspectsAndItems and clues[thePersonHere]
                    [thingBeingAskedAbout] not in PLACES):
                knownSuspectsAndItems.append(
                    clues[thePersonHere][thingBeingAskedAbout])
