...
CANVAS_WIDTH -= 1
# Leave room at the bottom for rows for the command info lines.
CANVAS_HEIGHT -= 5
...
def getCanvasString(canvasData, cx, cy):
    """Returns a multiline string of the line drawn in canvaData."""
    canvasStr = ''

    """canvasData is a dictionary with (x, y) tuple keys and values that
    are sets of 'W', 'A', 'S' and 'D' strings to show which
    directions the lines are drawn a each xy point."""
...
while True:  # Main program loop.
    ...
    if response == 'QUIT':
        ...
        print('draws a line going to right and sssdddwwwaaa draws a box.')
        print()
        ...
        try:
            ...
            with open(filename, 'w', encoding='utf-8') as fileObj:
                fileObj.write(''.join(moves) + '\n')
                fileObj.write(getCanvasString(canvas, None, None))
